...
var main = function (input) {
  ...
  if (gameMode == `waiting for username`) {
    userName = input;
    gameMode = `SPS`;
    myOutputValue = `Hello ${userName}! <br> <br> You are now ready to play. <br> <br> Paper, scissors, or stone?`;
  } else if (gameMode == `SPS`) {
    myOutputValue = spsGame(input);
  }

  return `${myOutputValue} <br> <br> GAME MODE: ${gameMode} <br> <br> Rounds played: ${roundCount}`;
};
...
var spsGame = function (playerHand) {
  ...
  if (WinnerResult == `Player loses`) {
  ...
  } else if (WinnerResult == `Player win`) {
    winCount = winCount + 1;
    roundCount = roundCount + 1;
    ...
    // input validation
    msg = `Sorry, your input was not recognized. You can only enter scissors, paper, or stone. Please try again.`;
  }
