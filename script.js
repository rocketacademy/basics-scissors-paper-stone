...
var main = function (input) {
  ...
  if (username == 0) {
    ...
    return (
      "Hi " +
      username +
      " :) <br> Please select game mode  (normal/reversed/korean/computer)"
    );
  } else {
    // select game mode
  ...
  if (input != "scissors" && input != "stone" && input != "paper") {
    output = "Please input again either (paper) (scissors) (stone)";
    return output;
...
var korean = function (input) {
  while (ultiWinner == 0) {
    ...
    if (firstWinner == 0 && gameStatus == "You Win!") {
      // User wins first round
      firstWinner = 1;
      return "You Win the first round! Input 2nd round guess";
    } else if (firstWinner == 0 && gameStatus == "You Lose!") {
      // Computer wins first round
      firstWinner = 2;
      return "You Lose the first round! Avoid guess the same as computer!";
    } else if (firstWinner == 0 && gameStatus == "Its a Draw!") {
      // First round draws
      return "Draw! input again";
    } else if (firstWinner == 1 && gameStatus == "Its a Draw!") {
      // User wins first round and draws second round, User is ultimate winner
      ultiWinner == 1;
      return "You are the ultimate Winner!";
    } else if (firstWinner == 2 && gameStatus == "Its a Draw!") {
      // Computer wins first round and draws second round, Computer is ultimate winner
      ultiWinner == 2;
      return "You lose! Computer is ultimate winner!";
    } else if (firstWinner == 1 && gameStatus != "Its a Draw!") {
      // User wins first round but not draw second round
      firstWinner = 0;
      return "You lose second round! Try again from first round";
    } else if (firstWinner == 2 && gameStatus != "Its a Draw!") {
      // Computer wins first round but not draw second round
      firstWinner = 0;
